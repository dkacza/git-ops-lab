...
# Wait for Flux to revert the drift.
# Our patch:      PRE_GEN → PRE_GEN+1
# Flux revert:    PRE_GEN+1 → PRE_GEN+2
echo "[INFO] Waiting for Flux to self-heal..."
REVERTED=false
...
if [[ "$REVERTED" != "true" ]]; then
    echo "[ERROR] Revert timeout after ${REVERT_TIMEOUT}s — Flux did not self-heal. Is prune/force enabled on the Kustomization?" >&2
    exit 1
fi

T_REVERT=$(date +%s)
REACTION=$((T_REVERT - T_START))
echo "[INFO] Drift reverted by Flux. Reaction time: ${REACTION}s"
...
mkdir -p "$RESULTS_DIR"
RESULTS_FILE="$RESULTS_DIR/flux-self-healing-$(date +%Y%m%d).csv"
