...
# Verify all Flux pods are Ready before killing
...
if [[ "$TOTAL" -eq 0 || "$READY" -ne "$TOTAL" ]]; then
    echo "[ERROR] Not all Flux pods are Ready ($READY/$TOTAL). Ensure Flux is healthy before measuring." >&2
    exit 1
fi

echo "[INFO] All $TOTAL Flux pods are Ready."
...
mkdir -p "$RESULTS_DIR"
RESULTS_FILE="$RESULTS_DIR/flux-failure-recovery-$(date +%Y%m%d).csv"
